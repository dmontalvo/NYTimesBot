...
if __name__ == "__main__":
    if os.path.exists("nytb_lock.txt"):
        print "Bot already running. Exiting."
        exit()
    i = open("nytb_lock.txt", 'w')
    try:
        global conn
        global curs
        ...
        starttime = time()
        #global logger
        #logger = open("/var/www/logs/nyt_bestsellers_bot/%s" % ctime(starttime), 'w')
        #logger.write('Started at: %s\n' % ctime(starttime))
        global logstring
        ...
        endtime = time()
        #logger.write("Ended at: %s\n" % ctime(endtime))
        #logger.write("Total run time: %s seconds\n" % (endtime - starttime))
        logstring += "Ended at: %s\n" % ctime(endtime)
        logstring += "Total run time: %s seconds\n" % (endtime - starttime)
        curs.execute("insert into logs (time, bot, logtype, data) values (%s, 'nyt_bestsellers_bot', 'logs', %s)", (ctime(starttime), logstring))
        conn.commit()
        os.remove("nytb_lock.txt")
        curs.close()

    except:
        error = traceback.format_exc()
        print error
        #el = open("/var/www/errors/nytb_errors.txt","a")
        #el.write("%s\n%s\n\n" % (ctime(starttime), error))
        curs.execute("insert into logs (time, bot, logtype, data) values (%s, 'nyt_bestsellers_bot', 'errors', %s)", (ctime(starttime), error))
        conn.commit()
        curs.close()
        os.remove("nytb_lock.txt")
